<header class="max-w-xl mx-auto mt-20 text-center">
    ...
    <div class="space-y-2 lg:space-y-0 lg:space-x-4 mt-4">
        <!--  Category -->
        <div class="relative flex lg:inline-flex items-center bg-gray-100 rounded-xl m-0">
            <div x-data="{show:false}" @click.away="show:false">
                <x-dropdown>
                    <x-slot name="trigger">
                        <button
                            @click="show=!show" class="inline-flex py-2 pl-3 pr-9 text-sm font-semibold w-32 text-left">
                    ...
                    @foreach($categories as $category)
                       ...
                       {{isset($currentCategory) && $currentCategory->id === $category->id ?'bg-blue-500 text-white':''}}
                        focus:text-white">{{ucwords($category->name)}}</a>
                    @endforeach
